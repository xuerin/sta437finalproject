...
ICA was applied to the PCA scores to extract statistically independent latent signals embedded within the traffic profiles. Intuitively, ICA identifies original components (inherent cause) as linear combinations for each $x_i$, beyond a mathematically rigorous uncovering of components (loadings) like in FA or PCA. While those loadings may certainly exist, they may not be source signals. Therefore, ICA assumes whitened inputs with identity covariance and therefore requires PCA preprocessing, like in FA [@hyvarinen2000ica]. Then, it mathematically follows that ICA yields the decomposition $S_{\text{PCA}} = A S_{\text{ICA}}$, where $S_{\text{ICA}}$ contains the source signals (scores) and $A$ is the mixing matrix (loadings). Unlike PCA and FA, ICA uses information on statistical independence and nongaussianity to recover source signals, meaning that it doesn't rely on the limited information in the covariance data matrix. It is reasonable to assume data is nongaussian, as while overall traffic sensors take in a mixture of data coming from various components and by the central limit theorem, any mixture of components will be more gaussian than the components themselves, but PC scores (in this case, original components) themselves are not guaranteed to be multivariate normally distributed (MVN), that is to say, non-gaussian. Then, ICA's goal is to maximize the nongaussianity of PC scores (components), this is the projection pursuit. ICA does not struggle with the factor rotation problem that is common in FA. 
