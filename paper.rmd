...
```{r plot-pca-loading, echo=FALSE, message=FALSE, warning=FALSE}

# Specify the two locations explicitly
...
# Side-by-side display using patchwork
pca_plot1 | pca_plot2
...
print(pca_summary)
```
```{r pca-boxplots-specific, fig.height=8, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r fa-loadings, echo=FALSE, message=FALSE, warning=FALSE}
library(patchwork)
...
```{r fa-boxplots-specific, fig.height=8, echo=FALSE, message=FALSE, warning=FALSE}
...
print(ica_summary)
```
```{r ica-boxplots, fig.height=8, echo=FALSE, message=FALSE, warning=FALSE}
# ---- Clean ICA boxplot function ----
...
# ---- Specific ICA locations ----
ica_locs <- c("Loc24", "Loc25", "Loc5")
...
```{r plot-anomalies-vs-normal, fig.width=12, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r barchart, echo=FALSE, message=FALSE, warning=FALSE}
df_long_summary <- summary_df %>%
...
```{r cluster-global-anomalies, echo=FALSE, message=FALSE, warning=FALSE}
...
rownames(all_anomalies) <- NULL
  ...
  cluster_plot <- ggplot(df_plot, aes(x = PC1, y = PC2, color = Cluster)) +
    ...
    labs(title = "Anomalies in PCA Space",
...
```{r three-tables-side-by-side, echo=FALSE, message=FALSE, warning=FALSE}
library(kableExtra)

# --- PCA table ---
pca_table <- data.frame(
  Location = names(location_results),
  PCA_Anomalies = sapply(location_results, \(x) length(x$pca_anom_days))
)

pca_kbl <- kbl(pca_table, booktabs=TRUE) %>%
  kable_styling(font_size=7) %>%
  add_header_above(c(" " = 1, "PCA" = ncol(pca_table)-1))

# --- FA table ---
fa_table <- data.frame(
  Location = names(location_results),
  FA_Anomalies = sapply(location_results, \(x) length(x$fa_anom_days))
)

fa_kbl <- kbl(fa_table, booktabs=TRUE) %>%
  kable_styling(font_size=7) %>%
  add_header_above(c(" " = 1, "FA" = ncol(fa_table)-1))

# --- ICA table ---
ica_table <- data.frame(
  Location = names(location_results),
  ICA_Anomalies = sapply(location_results, \(x) length(x$ica_anom_days))
)

ica_kbl <- kbl(ica_table, booktabs=TRUE) %>%
  kable_styling(font_size=7) %>%
  add_header_above(c(" " = 1, "ICA" = ncol(ica_table)-1))

# --- combine side-by-side ---
kables(
  list(pca_kbl, fa_kbl, ica_kbl),
  caption = "Side-by-Side Comparison of PCA, FA, and ICA Anomaly Counts",
  align = c("c", "c", "c")
)
```